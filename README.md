...
This project was foreseen as an opportunity to implement computer science concepts in the development of an art installation.

The installation depends on establishing propper connections between all of the interacting mediums.
...
To execute the installation, it is necessary to download and install:
    - Arduino IDE
    - Processing 3
    - Max 8
    - The Max library for Crow, created and developed by Monome.
    - The bonk~ object, created by Miller Puckette for PureData, and adapted to Max by Volker Böhm
    - The OscP5 library, created by Andreas Schlegel


The delivery method for the installation includes audio and video.

The audio is generated by pulses sent out of Crow that trigger analog synthesizers. This pulses are created in Max by a metro object that generates pulses at a specified rate. A pulse consist of a 5V square wave signal of a very short duration.

The graphics consist of an ellipse that changes size and color based on the amplitude of the incoming signal.
The size is dependent on amplitude changes, and the colors will be semi-randomly generated based on the transient information of the incoming audio.
The brightness of the color will depend on computing the randomly generated RGB values against the amplitude of the signal, so that the louder the music, the brighter the colors.

The software records Max's incoming audio into a wave file, and render png image frames that correspond to the graphic frames Processing generates.

These files can be imported into a video editing software to render a video file.

In my implementation, I am using Crow's output 1 to trigger a melodic line. Output 2 trigger a secondary melodic line that is offset by an eigth note. Output 3 activates a parameter that enhances the melodic line that comes from output 1 while the sequence is running. Output 4 activates a clock that can create subdivisions that trigger drums.

A Moog Sub37 is being triggered by the MIDI portion of the code and connectivity.
...
Make sure to connect the devices to the following pins:
...
2. Connect an additional cable coming from Arduino MEGA 2560 pin TX2 16, and going into Arduino UNO Digital PWM pin 2.
This is so that a Software Serial connection can be established between both Arduino's by using Arduino's Software Serial library.
This is necessary because the Joystick information is going to be sent to two different programs inside the computer (Max and Processing). Serial communication is a one to one communication, making the port that is used for one application unable to the other application.
This is despite the fact that the Arduino MEGA 2560 board has 4 serial ports on board, because the only serial port that can directly speak to the computer via USB is the default connection, unless cables are connected to the TX0 1 and RX 0 0 pins, which would break the USB serial connection.
...
1. Open the provided 'ArduinoMEGA_Main.ino' script.
...
3. In the toolbar, go to 'Tools' > 'Port' > '/dev/cu.usbmodem###### (Arduino Mega or Mega 2560)'. The hash symbols will be the number of the connected device. This connection will ensure that the code will be uploaded to the device connected to the selected port.
...
1. Open the provided 'ArduinoUNO_SerialThruPort.ino' script.

2. In the toolbar, go to 'Tools' > 'Board' > 'Arduino Uno' to configure the IDE to send information to the UNO board.

3. In the toolbar, go to 'Tools' > 'Port' > '/dev/cu.usbmodem######' (Arduino Uno)'. The hash symbols will be the number of the connected device. This connection will ensure that the code will be uploaded to the device connected to the selected port.
...
4. Open the provided 'Processing_audioGraphics.pde' sketch in Processing.
...
7. In the setup() function, change the hash to the value of correct USB port:
    String portName = Serial.list()[#];
This will ensure the right port is connected for the Serial transfer.
...
4. Open the provided 'Max_Host.maxpat' patch in Max.
...
12. Click on the 'Set Default Settings' to initiate the audio file in stereo (2 audio channels), at a sample rate of 48000kHz, and a bit depth resolution of 24 bits.
...
Arduino Button
    Start and Stop the Max audio sequence by instructing Crow to send out regular pulses from outputs 1, 2, and 4. This pulses will generate audio by triggering Analog Synthesizers.
    ...
    Output 3 is a 5V gate that opens and closes when the button is pressed. I used it to activate a parameter in the modular synthesizer that needs to be active while the instrument is playing, but deactivated when it is not.

Arduino Potentiometer
    Used to control the rate at which Max sends out pulses, which makes it a tempo control.
...
Arduino Joystick
    ...
    Pressing the Joystick's switch/clicker will also invert the colors of the ellipse and the background being drawn in Processing.
    ...
    Moving the Joystick's X and Y axis will also respectively control the X and Y axis positioning of the ellipse drawn in Processing.


Digital Controls

Max
    Max includes Bypass buttons for each of Crow's output channels to control which instruments are being triggered by the pulses generated by the metro object. It also includes a button to bypass the graphics by interrupting the signal that is output from Max to Processing.
...
Max
    If a file is created (via the 'Create File' button in the Max patch), Max will render a stereo audio file at a sample rato of 48000kHz, and a bit depth resolution of 24 bits.
...
Processing
    ...
    Processing will export the rendered frames to a folder named 'framesOut'. The numbering of these frames will be sequential and dependent on the number of iterations that the Processing draw() function has executed (this value is know in Processing as frameCount).

    These frames can then be imported into a Video Editor (like Premiere, or After Effects) to build the movie from frames. Audio can be imported too, synchronized, and exported as a standard video file (e.g. mov file). This file serves as a record of the performance, and can be used in all the traditional methods that video is used.
